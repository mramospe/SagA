...
saga::test::errors test_gravitational() {
  ...
  world.add_interaction<
      ...
      saga::physics::field_constant{1.f});

  world.configure([&](auto &container) {
    decltype(world)::particles_type::type_with_backend<saga::backend::CPU>
        cpu_container(number_of_particles);

    for (auto i = 0u; i < cpu_container.size(); ++i) {

      auto p = cpu_container[i];
    ...
    container = saga::to_backend<saga::backend::CUDA>(cpu_container);
  });
...
saga::test::errors test_collisions_elastic() {
  ...
  saga::world<saga::cuda::sf, saga::physics::sphere> world;
